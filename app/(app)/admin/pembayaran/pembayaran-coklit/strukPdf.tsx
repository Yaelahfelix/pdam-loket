...
type DRDTableProps = {
  ...
  isAdmin: boolean;
  tgl?: string;
  dataPrint?: DRDResponPembayaranAir[];
  subtitle?: string;
  headerlap1?: string;
  headerlap2?: string;
  alamat1?: string;
                  ...
                  <div className="border-t ">
                    <div className="flex justify-between font-bold">
                      ...
                      <div className="w-1/4 text-right">
                        {data.totalrekening.toLocaleString("id-ID")}
                      </div>
                    </div>
                  </div>
                </div>
...
const BayarHandler: React.FC<DRDTableProps> = (props) => {
  ...
  const [printData, setPrintData] = useState<DRDResponPembayaranAir[]>([]);
  useEffect(() => {
  ...
  const bayarHandler = async () => {
    ...
    setIsLoading(true);
    axios["post"](
      "/api/pembayaran/pembayaran-coklit",
      {
        data: props.data,
        detail: {
          user_id: session?.session.id,
          user_name: session?.session.nama,
          loket_id: session?.session.loketId,
          loket_name: session?.session.kodeloket,
          tglbayar: props.tgl,
  ...
  return (
    <div className="h-full">
      <div ref={componentRef} className="hidden-print">
        <ReportPrintComponent dataPrint={printData} {...props} />
      </div>
