...
export function ComboboxByPendaftaran({
  handler,
  allData,
  placeHolder = "Pilih dari pendaftaran...",
  isLoading = false,
}: {
  ...
  const fetchData = useCallback(async (searchQuery = "") => {
    try {
      ...
      const res = await axios.get(
        `/api/pembayaran/pembayaran-nonair/by-pendaftaran?q=${searchQuery}`,
        {
          headers: {
  ...
  const getSelectedData = () => {
    return data.find((k) => k.pendaftaranpel_id.toString() === value);
  };

  const getSelectedLabel = () => {
    const selected = data.find((k) => k.pendaftaranpel_id.toString() === value);
          ...
          <div className="max-h-60 overflow-auto">
            {loading && data.length === 0 ? (
              ...
              <ul>
                {data.map((data) => (
                  <li
                    key={data.pendaftaranpel_id}
                    className={cn(
                      "px-2 py-2 hover:bg-gray-100 cursor-pointer rounded-md",
                      value === data.pendaftaranpel_id.toString() &&
                        "bg-blue-50"
                    )}
                    onClick={() => {
                      setValue(data.pendaftaranpel_id.toString());
                      setIsOpen(false);
                    }}
                  >
